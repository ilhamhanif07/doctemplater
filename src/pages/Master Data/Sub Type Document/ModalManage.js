  ...
  const clickInsert = async () => {
    ...
    if (show.isConfirmed) {
      if (
        ...
        if (res.status === "000") {
          toast.success(
            `Add Sub Type Document ${inputSubtypeDocName} Success: ${res.errMsg}`
          );
        } else {
          toast.error(
            `Add Sub Type Document ${inputSubtypeDocName} Failed: ${res.errMsg}`
          );
        }
          ...
          <div className="row mb-2">
            <div className="col-md-12">
              ...
              >
                <Input
                  onChange={(e) => {
                    setInputSubtypeDocName(e.target.value);
                  }}
