...
import { getDocumentSubType, getDocumentType, getMappingApproval, getUserCompany, getJabatan } from '../../../GQL/query';
...
function MappingApproval() {
	const column = [
		...
		},
		{
			...
			fixedRight: true,
			position: 'center',
			renderCell: (data) => <Button variant="text" color="primary">Edit</Button>
		},
	]
				...
				<Paper sx={{ pl: 2, pr: 2, pt: 1, pb: 1 }}>
					...
					<Collapse in={isCollapse}>
						...
						<div className="row">
							...
							<div className="col-md-6 mb-1">
								<Select
									options={[{ label: 'Level 1', value: '1' }, { label: 'Level 2', value: '2' }, { label: 'Level 3', value: '3' }]}
									placeholder="Level..."
				...
				<Paper sx={{ p: 2, mt: 1 }}>
					...
					<div className="row mt-1">
						...
						<div className="col-md-2">
							<Button variant="contained" color="primary" startIcon={<Icon>add</Icon>} style={{ width: '100%' }}>Tambah</Button>
						</div>
					</div>
					...
					<DataTable
						...
						data={
							dataTable.filter((item) => {
								let flt1 = false, flt2 = false, flt3 = false, flt4 = false, flt5 = false;

								if (fltSearch === null) {
									flt1 = true;
								} else {
									if (item?.level?.toUpperCase().includes(fltSearch.toUpperCase())
										|| item?.company_name?.toUpperCase().includes(fltSearch.toUpperCase())
										|| item?.company_alias?.toUpperCase().includes(fltSearch.toUpperCase())
										|| item?.document_type_name?.toUpperCase().includes(fltSearch.toUpperCase())
										|| item?.position_name?.toUpperCase().includes(fltSearch.toUpperCase())) {
										flt1 = true;
									}
								}
								...
								if (fltLevel === null) {
									...
									if (item?.level?.toUpperCase().includes(fltLevel.label.toUpperCase())) {
										flt2 = true;
									}
								}
								...
								if (fltJabatan === null) {
									...
									if (item?.position_name.toUpperCase().includes(fltJabatan.toUpperCase())) {
										flt4 = true;
									}
								}
								...
								if (fltUserCompany === null) {
									...
									if (item?.company_name.toUpperCase().includes(fltUserCompany.label.toUpperCase())) {
										flt5 = true;
									}
								}
